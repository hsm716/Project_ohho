...
public class QuestData : MonoBehaviourPunCallbacks
{
    ...
    private void Update()
    {
        if (myPlayer.PV.IsMine)
        {
            ...
            slimeSlider.value = 100 * forestSpirit_CurCount / forestSpirit_MaxCount;    //0번 : 숲
            Slime.text = forestSpirit_CurCount + " / " + forestSpirit_MaxCount;

            twigSlider.value = (twigCurCount / (float)twigMaxCount);    //1번 : 사막
            ...
            twigSlider_forge.value = (twigforgeCurCount / (float)twigforgeMaxCount);    //3번 : 포지
            twig_forge.text = twigforgeCurCount + " / " + twigforgeMaxCount;

            OccupiedSlider.value = (OccupiedValue_cur / OccupiedValue_max);     //4번 : 눈
